...
<%
  ...
  CreateUI('pg2_1_1.asp', 'mainform', false, true,
      new Array(
        'submit', '查詢',
        'submit', '清除查詢條件'
      ),
...
<%
  if (!IsNull(Rcv.Item('_DO_QUERY')) || !IsNull(Rcv.Item('_BrowseLocation')))
  {
    ...
    if (!IsNull(Session('SEARCH2_RowGuids')))
    {
      Session('SEARCH2_RowGuids').Close();
      Session('SEARCH2_RowGuids') = null;
    }

    Session('SEARCH2_RowGuids') =
        SQLExecute("select " +
          " DDQS_DOC.rowguid " +
