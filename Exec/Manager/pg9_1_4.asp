...
*/
  ...
  CreateUI('pg9_1_4.asp', 'mainform', false, true,
      ...
      new Array(
        ...
        new Array('sqlcombobox', '使用者:', 'USER_ID collate Chinese_Taiwan_Stroke_CI_AS', 'rw', '', 'user_id', 'user_name',
            " select user_id, USER_ID collate Chinese_Taiwan_Stroke_CI_AS + ' ' + SECTN_CHI_NM + '.' + EMPL_NM user_name " +
            " from webap_user, QICT105, QICT010 " +
            " where user_role_id<>'0' " +
            " and EMPL_CD = USER_ID collate Chinese_Taiwan_Stroke_CI_AS" +
  ...
  if (IsNull(Rcv.Item('USER_ID')))
  {
    BrowseData("select " +
      ...
      " dbo.GetChDateStr(max(EDATE)) as LAST_EDATE, USER_ID collate Chinese_Taiwan_Stroke_CI_AS as rowguid " +
      " from DDQS_LOG, QICT105, QICT010 " +
      " where EMPL_CD = USER_ID collate Chinese_Taiwan_Stroke_CI_AS" +
      " and ORG_SECTN = ORG_CD+SECTN_CD " +
      " and QICT010.DIVISION_CD = '00' " +
      " and " + Rcv.GetSqlSearchWhereConStr() +
      " group by DOC_TYPE, ACCESS_REC, USER_ID collate Chinese_Taiwan_Stroke_CI_AS+ ' ' + SECTN_CHI_NM + '.' + EMPL_NM, USER_ID collate Chinese_Taiwan_Stroke_CI_AS " +
