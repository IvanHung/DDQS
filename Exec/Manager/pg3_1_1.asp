...
<%
  ...
  CreateUI('pg3_1_1.asp', 'mainform', false, true,
      new Array(
        'submit', '查詢',
        'submit', '清除查詢條件'
      ),
      new Array(
        ...
        new Array('sqlcombobox', '行政訴訟項目:', '_INDEX2_idx', 'rw', Rcv.Item('_INDEX2_idx'), 'rowguid', 'INDEX2', "select * from DDQS_INDEX2 " +
        ...
        new Array('customcombobox', '為逕提行政訴訟案件:', 'PEOPLE_SUBMIT', 'rw', '', new Array('', '全部', 'Y', '是', 'N', '否'), false, ''),
...
<%
  if (!IsNull(Rcv.Item('_DO_QUERY')) || !IsNull(Rcv.Item('_BrowseLocation')))
  {
    ...
    Session('SEARCH3_WhereCon') = WhereCon;
    Session('SEARCH3_Order') = " SEND_DATE desc ";

    if (!IsNull(Session('SEARCH3_RowGuids')))
    {
      Session('SEARCH3_RowGuids').Close();
      Session('SEARCH3_RowGuids') = null;
    }

    Session('SEARCH3_RowGuids') =
        SQLExecute("select " +
          " DDQS_DOC.rowguid " +
          " from DDQS_DOC, DDQS_INDEX1 " +
          " where " + Session('SEARCH3_WhereCon') +
          " and DDQS_DOC.INDEX1_idx=DDQS_INDEX1.rowguid " +
          " order by " + Session('SEARCH3_Order'));
  }

  if (!IsNull(Session('SEARCH3_WhereCon')))
  {
    BrowseData("select " +
      ...
      new Array(
          '發文日期', '文號', '統一編號', '稅目', '行政訴訟項目', '關鍵字', '為逕提行政訴訟案件'),
      'pg3_1_1_d.asp', false, '', '', '');
  }
%>
