...
<%
  ...
  if (Rcv.Item('_action') == '新增')
  {
    ...
    LogWRITE(Rcv.Item('DOCNO'), 2);

    ShowMessage('新增作業完成.', new Array('繼續新增', "location.href='pg2_1_2_n.asp'", '回瀏覽頁', "location.href='pg2_1_2.asp'"));
  }
  else if (Rcv.Item('_action') == '修改')
  {
    ...
    ShowMessage('資料修改完成.', new Array('回明細頁', "location.href='pg2_1_2_d.asp?rowguid=" + Rcv.Item('rowguid') + "'"));
  }
  else if (Rcv.Item('_action') == '刪除')
  {
    SQLExecute("delete from DDQS_DOC where rowguid='" + Rcv.Item('rowguid') + "'");

    ShowMessage('資料刪除完成.', new Array('回瀏覽頁', "location.href='pg2_1_2.asp'"));
  }
%>
