...
<%
/*
  if (!IsNull(Rcv.Item('file_name')))
  {
    var HasFile;

    try
    {
      HasFile = FileExists(ToRealFilePath("/") + "\\..\\ConvertArea\\" + Rcv.Item('file_name') + '.txt');
    }
    catch (e)
    {
      HasFile = false;
...
<table border="0" class=pagetable cellpadding="1" cellspacing="1" height="100%" width="100%">
  ...
  <tr>
    <td nowrap align="middle" valign="center">
      <font color=blue><b>檔案轉換中, 請稍候...</b></font>
    </td>
  </tr>
</table>
</body>
...
<%
    ...
    {
      Rcv.SetItem('DOC', LoadFromFile(ToRealFilePath("/") + "\\..\\ConvertArea\\" + Rcv.Item('file_name') + '.txt'));
      DeleteFile(ToRealFilePath("/") + "\\..\\ConvertArea\\" + Rcv.Item('file_name') + '.txt');
    }
  }
*/
%>
...
<%
  if (Rcv.Item('rowguid') != '')
  {
    ...
    CreateUI('pg1_1_2_r.asp', 'mainform', true, false,
        new Array(
            "location.href='pg1_1_1_p.asp?rowguid=" + Sav.Item('rowguid') + "'", '列印',
            "location.href='pg1_1_2_n.asp'", '新增復查決定書',
            "submit", '修改',
            "if (window.confirm('您確認要刪除此筆資料嗎?')) location.href='pg1_1_2_r.asp?_action=刪除&rowguid=" + Sav.Item('rowguid') + "';", '刪除',
            "location.href='pg1_1_2.asp'", '回瀏覽頁'
        ),
        new Array(
          // 案件內容
          ...
          new Array('textarea', '復查項目:', 'INDEX2_LIST', 'rw', '', 5, 80, 2000, ''),
  ...
  else
  {
    ...
    CreateUI('pg1_1_2_r.asp', 'mainform', false, true,
        new Array(
            'submit', '新增',
            "location.href='pg1_1_2.asp'", '回瀏覽頁'
        ),
        new Array(
          ...
          new Array('text', '統一編號:', 'IDNO', 'rw', '', 10, 10, ''),
          new Array('chdate', '發文日期:', 'SEND_DATE', 'rw', '', ''),
          new Array('sqlcombobox', '稅目:', 'INDEX1_idx', 'rw', '', 'rowguid', 'INDEX1', "select * from DDQS_INDEX1 order by INDEX1", true, "onChange='mainform.action=\"pg1_1_2_d.asp\"; mainform.submit();'"),
          new Array('sqlcombobox', '加入復查項目:', '_INDEX2_idx', 'rw', '', 'rowguid', 'INDEX2', "select '[清除復查項目]' as INDEX2, '[CLEAR]' as rowguid union select INDEX2, cast(rowguid as varchar(40)) as rowguid from DDQS_INDEX2 " +
              "where " + (IsNull(Rcv.Item('INDEX1_idx'))?"1=0":"INDEX1_rowguid='" + Rcv.Item('INDEX1_idx') + "'") + " order by INDEX2", true, "onChange='mainform.action=\"pg1_1_2_d.asp\"; mainform.submit();'"),
          ...
          new Array('sqlcombobox', '加入關鍵字:', '_KEYS', 'rw', '-', 'KEYS', 'KEYS', "select '[清除關鍵字]' as KEYS union select KEYS from DDQS_KEYS " +
              "where " + (IsNull(Rcv.Item('_INDEX2_idx'))?"1=0":"INDEX2_rowguid='" + Rcv.Item('_INDEX2_idx') + "'") + " order by KEYS", true, "onChange='mainform.action=\"pg1_1_2_d.asp\"; mainform.submit();'"),
          new Array('text', '加入自訂關鍵字:', '_CUSTOM_KEY', 'rw', '', 40, 40, "><input type=\"button\" class=\"button\" value=\"加入\" onclick='mainform.action=\"pg1_1_2_d.asp\"; mainform.submit();' "),
